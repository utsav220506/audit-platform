  ...
  try {
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS audit_logs (
        ...
        action        TEXT CHECK(action IN ('CREATE_DOC','EDIT_DOC','VIEW_DOC','GENERATE_REPORT','SHARE_DATA')) NOT NULL,
...
app.post('/documents', auth, async (req, res) => {
  try {
    ...
    try {
      ...
      await client.query(`INSERT INTO documents(document_id,title,created_by) VALUES($1,$2,$3)`, [docId,title.trim(),req.user.userId]);
      await client.query(`INSERT INTO document_versions(version_id,document_id,content,created_by,hash,version_number) VALUES($1,$2,$3,$4,$5,1)`, [verId,docId,content,req.user.userId,hash]);
...
app.post('/documents/:id/edit', auth, async (req, res) => {
  try {
    ...
    try {
      await client.query('BEGIN');
      await client.query(`INSERT INTO document_versions(version_id,document_id,content,created_by,hash,version_number) VALUES($1,$2,$3,$4,$5,$6)`, [verId,req.params.id,content,req.user.userId,hash,newVersion]);
      await client.query('COMMIT');
    } catch(e) { await client.query('ROLLBACK'); throw e; }
    finally { client.release(); }
...
app.get('/documents/:id/versions', auth, async (req, res) => {
  try {
...
app.get('/documents/:id/compare', auth, async (req, res) => {
  try {
...
app.post('/reports/generate', auth, requireRole('TEACHER','HOD'), async (req, res) => {
  try {
...
app.get('/reports', auth, requireRole('TEACHER','HOD'), async (req, res) => {
  try {
...
app.get('/reports/:id', auth, requireRole('TEACHER','HOD'), async (req, res) => {
  try {
...
app.post('/share', auth, requireRole('TEACHER','HOD'), async (req, res) => {
  try {
    ...
    const shareId=uuid();
    await pool.query(`INSERT INTO share_logs(share_id,shared_by,shared_to,document_id) VALUES($1,$2,$3,$4)`, [shareId,req.user.userId,sharedTo||null,documentId]);
    await auditLog('SHARE_DATA', req.user.userId, documentId, 'document', { sharedTo });
    res.status(201).json({ shareId, message:'Document shared successfully' });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.get('/audit-logs', auth, requireRole('TEACHER','HOD'), async (req, res) => {
  try {
...
app.get('/users', auth, requireRole('TEACHER','HOD'), async (req, res) => {
  try {
